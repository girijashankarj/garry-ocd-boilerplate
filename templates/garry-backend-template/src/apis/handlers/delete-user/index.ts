...
type DeleteResponse = ApiResult<{ id: string }>;

const requestSchema = requestSchemaJson as JSONSchemaType<IdParam>;
const responseSchema = responseSchemaJson as JSONSchemaType<DeleteResponse>;
...
const handler = withWrap(async (req, res) => {
  try {
    ...
    const responsePayload: DeleteResponse = { ok: true, data: postProcessed };
    validateResponse(responseSchema, responsePayload);

    loggerInfo(
      INFO_MESSAGES.REQUEST_OK,
      { id: requestPayload.id },
      OPERATIONS.USERS_DELETE,
